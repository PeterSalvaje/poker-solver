...
private:
    ...
    std::map<ActionType, double> actionProbabilities = {
        {ActionType::Fold, 0.2},
        {ActionType::Call, 0.5},
        {ActionType::Raise, 0.3},
        {ActionType::Bet, 0.6},
        {ActionType::Check, 0.4}
    };

    std::map<ActionType, bool> actionLocked = {
        {ActionType::Fold, false},
        {ActionType::Call, false},
        {ActionType::Raise, false},
        {ActionType::Bet, false},
        {ActionType::Check, false}
    };
public:
    ...
    void updateActionProbabilities(GameTreeNode* node, const std::map<ActionType, double>& newProbabilities, std::map<ActionType, bool>& lockedActions);
    std::map<ActionType, bool>& getActionLocked() {
        return actionLocked;
    }
    std::map<ActionType, double> getActionProbabilities() const {
