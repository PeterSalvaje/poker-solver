...
public:
    ActionNode(vector<GameActions> actions,
               ...
               std::map<ActionType, double> actionProbabilities); // Añadir el parámetro
    ~ActionNode();
...
private:
    ...
    std::map<ActionType, double> actionProbabilities; // Mapa para almacenar las probabilidades de cada acción
public:
    ...
    void setActionProbabilities(const std::map<ActionType, double>& newProbabilities) {
        actionProbabilities = newProbabilities;
    }
