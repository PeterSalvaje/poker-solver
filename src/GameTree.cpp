...
GameTree::GameTree(Deck deck,
 ...
 // shared_ptr<ActionNode> node = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(), current_player, round, (double) rule.get_pot(), nullptr);
    shared_ptr<ActionNode> node = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(), current_player, round, (double) rule.get_pot(), nullptr, getActionProbabilities());
...
void GameTree::updateActionProbabilities(GameTreeNode* node,
                                         const std::map<ActionType, double>& newProbabilities,
                                        std::map<ActionType, bool>& lockedActions) {
    // 1. Actualizar las probabilidades del nodo actual si es un ActionNode
    if (node->getType() == GameTreeNode::ACTION) {
        ActionNode* actionNode = dynamic_cast<ActionNode*>(node);
        std::map<ActionType, double> currentProbabilities = actionNode->getActionProbabilities(); // Obtener las probabilidades actuales del nodo
        for (auto const& [action, probability] : newProbabilities) {
            if (!lockedActions[action]) {
                currentProbabilities[action] = probability;
            }
        }
        actionNode->setActionProbabilities(currentProbabilities); // Asignar el mapa actualizado al nodo
    }

    // 2. Propagar los cambios a los nodos hijos (si los hay)
    if (node->getType() == GameTreeNode::ACTION) {
        ActionNode* actionNode = dynamic_cast<ActionNode*>(node);
        for (const auto& childPtr : actionNode->getChildrens()) {
            GameTreeNode* child = childPtr.get();
            if (child->getType() == GameTreeNode::ACTION) {
                ActionNode* actionChild = dynamic_cast<ActionNode*>(child);
                updateActionProbabilities(child, actionChild->getActionProbabilities(), lockedActions);
...
void GameTree::buildChance(shared_ptr<ChanceNode> root,Rule rule){
    ...
    if(rule.oop_commit == rule.ip_commit && rule.oop_commit == rule.stack) {
        ...
        // Obtener el padre del nodo ChanceNode y castearlo a ActionNode
        shared_ptr<ActionNode> parentActionNode = dynamic_pointer_cast<ActionNode>(root->getParent());

        // Verificar si el casting fue exitoso
        if (parentActionNode) {
            // Crea el ActionNode con las probabilidades del padre
            one_node = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(), 1, GameTreeNode::intToGameRound(rule.current_round), (double) rule.get_pot(), parentActionNode, parentActionNode->getActionProbabilities());
        } else {
            // Manejar el caso en que el padre no sea un ActionNode (puedes usar probabilidades por defecto)
            // Crea el ActionNode con probabilidades por defecto
            std::map<ActionType, double> defaultProbabilities = {
                {ActionType::Fold, 0.2},  // 20% de probabilidad de Fold por defecto
                {ActionType::Call, 0.5},  // 50% de probabilidad de Call por defecto
                {ActionType::Raise, 0.3},  // 30% de probabilidad de Raise por defecto
                {ActionType::Bet, 0.6},  // 60% de probabilidad de Bet por defecto
                {ActionType::Check, 0.4}  // 40% de probabilidad de Check por defecto
            };
            one_node = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(), 1, GameTreeNode::intToGameRound(rule.current_round), (double) rule.get_pot(), root, defaultProbabilities);
        }
    }
    if(rule.current_round > 3)throw runtime_error(tfm::format("current round not valid : %d",rule.current_round));
    ...
    for (string action : possible_actions) {
        if (action == "check") {
            ...
            if ((last_action == "call" && root->getParent() != nullptr && root->getParent()->getParent() == nullptr) || check_times >= 1) {
                ...
                nextnode = make_shared<ActionNode>(vector<GameActions>(),vector<shared_ptr<GameTreeNode>>() , nextplayer, GameTreeNode::intToGameRound(rule.current_round), (double) rule.get_pot(), root, root->getActionProbabilities());
            } else {
                nextnode = make_shared<ActionNode>(vector<GameActions>(),vector<shared_ptr<GameTreeNode>>() , nextplayer, GameTreeNode::intToGameRound(rule.current_round), (double) rule.get_pot(), root, root->getActionProbabilities());
            }
            ...
            for(double one_betting_size:bet_sizes){
                ...
                shared_ptr<GameTreeNode> nextnode = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(),nextplayer,GameTreeNode::intToGameRound(rule.current_round),(double) rule.get_pot(),root, root->getActionProbabilities());
            ...
            if(root->getParent() == nullptr) {
                nextnode = make_shared<ActionNode>(vector<GameActions>(), vector<shared_ptr<GameTreeNode>>(), nextplayer, GameTreeNode::intToGameRound(rule.current_round), (double) nextrule.get_pot(), root, root->getActionProbabilities());
...
GameTree::generateActionNode(json meta, vector<string> childrens_actions, vector<json> childrens_nodes, const string& round,
    ...
    shared_ptr<ActionNode> actionNode = make_shared<ActionNode>(actions,childrens,player,game_round,pot, parent, defaultProbabilities);
