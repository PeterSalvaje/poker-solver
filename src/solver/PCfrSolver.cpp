...
void PCfrSolver::retrain(GameTreeNode* node) {
    // Obtener las probabilidades de alcance del nodo actual
    vector<vector<float>> reach_probs = this->getReachProbs();

    // Obtener el jugador actual
    int player_id = dynamic_cast<ActionNode*>(node)->getPlayer();

    // Determinar el tipo de nodo
    GameTreeNode::GameTreeNodeType nodeType = node->getType();

    // Crear el shared_ptr según el tipo de nodo
    shared_ptr<GameTreeNode> sharedNode;
    switch (nodeType) {
    case GameTreeNode::ACTION:
        sharedNode = std::make_shared<ActionNode>(*dynamic_cast<ActionNode*>(node));
        break;
    case GameTreeNode::CHANCE:
        sharedNode = std::make_shared<ChanceNode>(*dynamic_cast<ChanceNode*>(node));
        break;
    case GameTreeNode::SHOWDOWN:
        sharedNode = std::make_shared<ShowdownNode>(*dynamic_cast<ShowdownNode*>(node));
        break;
    case GameTreeNode::TERMINAL:
        sharedNode = std::make_shared<TerminalNode>(*dynamic_cast<TerminalNode*>(node));
        break;
    default: break;
    }
    qDebug() << "RP 1: " << reach_probs[1 - player_id];
    for(int i = 0;i < this->iteration_number;i++){
        this->round_deal = vector<int>{-1,-1,-1,-1};
        //cfr(player_id, sharedNode, reach_probs[1 - player_id], i, this->initial_board_long,0);
        cfrSubtree(player_id, std::static_pointer_cast<GameTreeNode>(sharedNode), reach_probs[1 - player_id], i, this->initial_board_long,0); // Llamar a cfrSubtree()
    }
}

vector<float> PCfrSolver::cfrSubtree(int player, shared_ptr<GameTreeNode> node, const vector<float>& reach_probs, int iter, uint64_t current_board,int deal) {
    // Actualizar las probabilidades de alcance con las probabilidades del nodo actual
    vector<vector<float>> newReachProbs = this->getReachProbs(); // Obtener las probabilidades iniciales
    if (node->getType() == GameTreeNode::ACTION) {
        ActionNode* actionNode = dynamic_cast<ActionNode*>(node.get());
        std::map<ActionType, double> actionProbabilities = actionNode->getActionProbabilities(); // Obtener las probabilidades de acción del nodo
        for (int i = 0; i < newReachProbs[1 - player].size(); ++i) {
            newReachProbs[1 - player][i] = static_cast<float>(actionProbabilities[ActionType::Fold]); // Actualizar la probabilidad de Fold
            newReachProbs[1 - player][i] = static_cast<float>(actionProbabilities[ActionType::Call]); // Actualizar la probabilidad de Call
            newReachProbs[1 - player][i] = static_cast<float>(actionProbabilities[ActionType::Raise]); // Actualizar la probabilidad de Raise
            //newReachProbs[1 - player][i] *= static_cast<float>(actionProbabilities[ActionType::Bet]); // Actualizar la probabilidad de Bet
            //newReachProbs[1 - player][i] *= static_cast<float>(actionProbabilities[ActionType::Check]); // Actualizar la probabilidad de Check
            // ... (actualizar las probabilidades de Bet y Check si están disponibles) ...
        }
    }
    qDebug() << "RP 2: " << newReachProbs[player];
    qDebug() << "RP 3: " << newReachProbs[1 - player];
    switch(node->getType()) {
        case GameTreeNode::ACTION: {
            shared_ptr<ActionNode> action_node = std::dynamic_pointer_cast<ActionNode>(node);
            return actionUtility(player, action_node, newReachProbs[1 - player], iter, current_board,deal); // Pasar newReachProbs[player]
        }
        case GameTreeNode::SHOWDOWN: {
            shared_ptr<ShowdownNode> showdown_node = std::dynamic_pointer_cast<ShowdownNode>(node);
            return showdownUtility(player, showdown_node, newReachProbs[1 - player], iter, current_board,deal); // Pasar newReachProbs[player]
        }
        case GameTreeNode::TERMINAL: {
            shared_ptr<TerminalNode> terminal_node = std::dynamic_pointer_cast<TerminalNode>(node);
            return terminalUtility(player, terminal_node, newReachProbs[1 - player], iter, current_board,deal); // Pasar newReachProbs[player]
        }
        case GameTreeNode::CHANCE: {
            shared_ptr<ChanceNode> chance_node = std::dynamic_pointer_cast<ChanceNode>(node);
            return chanceUtility(player, chance_node, newReachProbs[1 - player], iter, current_board,deal); // Pasar newReachProbs[player]
        }
        default:
            throw runtime_error("node type unknown");
    }
